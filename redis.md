...
1. 下载[Redis-x64-3.2.100.zip](https://github.com/microsoftarchive/redis/releases/download/win-3.2.100/Redis-x64-3.2.100.zip)安装包，解压到C 盘，解压后，将文件夹重新命名为 **redis**    
2. 启动redis
...
Redis支持五种数据类型：string（字符串），hash（字典），list（列表），set（集合）及zset(sorted set：有序集合)。  
...
GET testkey
DEL testkey
#Hash（哈希）,hash 是一个键值(key=>value)对集合。是一个 string 类型的 field 和 value 的映射表
HMSET testkey field1 "Hello" field2 "World"
HGET testkey field1   #"Hello"
HGET testkey field2   #"World"
DEL testkey
#List（列表）,Redis 列表是简单的字符串列表，按照插入顺序排序。你可以添加一个元素到列表的头部（左边）或者尾部（右边）。
lpush testkey mongodb
lpush testkey rabitmq
rpush testkey tomcat
lrange testkey 0 10
#Set（集合）
sadd testkey mongodb
sadd testkey rabitmq
sadd testkey rabitmq  #第二次插入的元素将被忽略
smembers testkey
#zset(sorted set：有序集合),每个元素都会关联一个double类型的分数。redis正是通过分数来为集合中的成员进行从小到大的排序。zset的成员是唯一的,但分数(score)却可以重复。
# zadd key score member 
zadd testkey 0 mongodb
zadd testkey 1 rabitmq
zadd testkey 0 rabitmq  #第二次插入的元素将被忽略,但是会更新score
zadd testkey 0.5 tomcat
ZRANGEBYSCORE testkey 0 1000  #根据score从小到大排序
```
